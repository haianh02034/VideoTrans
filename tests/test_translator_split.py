...
class TestTranslatorIndexConstants:
    ...
    def test_google_index(self):
        from phiendichvideo.translator import GOOGLE_INDEX
        assert GOOGLE_INDEX == 0

    def test_microsoft_index(self):
        from phiendichvideo.translator import MICROSOFT_INDEX
        assert MICROSOFT_INDEX == 1

    def test_m2m100_index(self):
        from phiendichvideo.translator import M2M100_INDEX
        assert M2M100_INDEX == 2

    def test_chatgpt_index(self):
        from phiendichvideo.translator import CHATGPT_INDEX
        assert CHATGPT_INDEX == 3

    def test_deepseek_index(self):
        from phiendichvideo.translator import DEEPSEEK_INDEX
        assert DEEPSEEK_INDEX == 4

    def test_gemini_index(self):
        from phiendichvideo.translator import GEMINI_INDEX
        assert GEMINI_INDEX == 5

    def test_zhipuai_index(self):
        from phiendichvideo.translator import ZHIPUAI_INDEX
        assert ZHIPUAI_INDEX == 6

    def test_azuregpt_index(self):
        from phiendichvideo.translator import AZUREGPT_INDEX
        assert AZUREGPT_INDEX == 7

    def test_locallm_index(self):
        from phiendichvideo.translator import LOCALLLM_INDEX
        assert LOCALLLM_INDEX == 8

    def test_openrouter_index(self):
        from phiendichvideo.translator import OPENROUTER_INDEX
        assert OPENROUTER_INDEX == 9

    def test_siliconflow_index(self):
        from phiendichvideo.translator import SILICONFLOW_INDEX
        assert SILICONFLOW_INDEX == 10

    def test_ai302_index(self):
        from phiendichvideo.translator import AI302_INDEX
        assert AI302_INDEX == 11

    def test_qwenmt_index(self):
        from phiendichvideo.translator import QWENMT_INDEX
        assert QWENMT_INDEX == 12

    def test_zijie_index(self):
        from phiendichvideo.translator import ZIJIE_INDEX
        assert ZIJIE_INDEX == 13

    def test_tencent_index(self):
        from phiendichvideo.translator import TENCENT_INDEX
        assert TENCENT_INDEX == 14

    def test_baidu_index(self):
        from phiendichvideo.translator import BAIDU_INDEX
        assert BAIDU_INDEX == 15

    def test_deepl_index(self):
        from phiendichvideo.translator import DEEPL_INDEX
        assert DEEPL_INDEX == 16

    def test_deeplx_index(self):
        from phiendichvideo.translator import DEEPLX_INDEX
        assert DEEPLX_INDEX == 17

    def test_ali_index(self):
        from phiendichvideo.translator import ALI_INDEX
        assert ALI_INDEX == 18

    def test_libre_index(self):
        from phiendichvideo.translator import LIBRE_INDEX
        assert LIBRE_INDEX == 19

    def test_minimax_index(self):
        from phiendichvideo.translator import MINIMAX_INDEX
        assert MINIMAX_INDEX == 20

    def test_xiaomi_index(self):
        from phiendichvideo.translator import XIAOMI_INDEX
        assert XIAOMI_INDEX == 21

    def test_camb_index(self):
        from phiendichvideo.translator import CAMB_INDEX
        assert CAMB_INDEX == 22

    def test_transapi_index(self):
        from phiendichvideo.translator import TRANSAPI_INDEX
...
class TestTranslatorGetSourceTargetCode:
    ...
    def test_google_channel(self):
        from phiendichvideo.translator import get_source_target_code, GOOGLE_INDEX
    ...
    def test_ai_channel(self):
        from phiendichvideo.translator import get_source_target_code, CHATGPT_INDEX
        src, tgt = get_source_target_code(
    ...
    def test_baidu_channel(self):
        from phiendichvideo.translator import get_source_target_code, BAIDU_INDEX
        src, tgt = get_source_target_code(
...
class TestTranslatorIsAllowTranslate:
    ...
    def test_google_always_allowed(self):
        from phiendichvideo.translator import is_allow_translate, GOOGLE_INDEX
        assert is_allow_translate(translate_type=GOOGLE_INDEX) is True

    def test_microsoft_always_allowed(self):
        from phiendichvideo.translator import is_allow_translate, MICROSOFT_INDEX
