...
class TestPrepareAudioImports:
    def test_vocal_bgm_importable(self):
        from phiendichvideo.process.prepare_audio import vocal_bgm
        assert callable(vocal_bgm)

    def test_vocal_bgm_spleeter_importable(self):
        from phiendichvideo.process.prepare_audio import vocal_bgm_spleeter
    ...
    def test_cam_speakers_importable(self):
        from phiendichvideo.process.prepare_audio import cam_speakers
        assert callable(cam_speakers)

    def test_pyannote_speakers_importable(self):
        from phiendichvideo.process.prepare_audio import pyannote_speakers
        assert callable(pyannote_speakers)

    def test_reverb_speakers_importable(self):
        from phiendichvideo.process.prepare_audio import reverb_speakers
        assert callable(reverb_speakers)

    def test_built_speakers_importable(self):
        from phiendichvideo.process.prepare_audio import built_speakers
        assert callable(built_speakers)

    def test_write_log_importable(self):
        from phiendichvideo.process.prepare_audio import _write_log
        assert callable(_write_log)

    def test_all_9_names_importable(self):
        from phiendichvideo.process import prepare_audio
...
class TestAssignSpeakers:
    def test_assign_empty_diarizations(self):
        from phiendichvideo.process._audio_speakers import _assign_speakers
...
class TestNormalizeDiarizations:
    def test_normalizes_and_remaps(self):
        from phiendichvideo.process._audio_speakers import _normalize_diarizations
        raw = [
