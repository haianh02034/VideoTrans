...
# USE_CUDA=false -> base-false (CPU), USE_CUDA=true -> base-true (GPU)
...
# Khai báo lại để dùng được ARG toàn cục bên trong stage này
ARG USE_CUDA

# Chép riêng manifest trước để lớp cài thư viện được cache lại:
# sửa mã nguồn sẽ không phải cài lại toàn bộ dependencies.
COPY pyproject.toml ./

RUN if [ "$USE_CUDA" = "true" ]; then echo ">>> CUDA" && uv pip install --system -r pyproject.toml --all-extras && uv pip install --system torch==2.7.1 torchaudio==2.7.1 --index-url https://download.pytorch.org/whl/cu128 && uv pip install --system nvidia-cublas-cu12 nvidia-cudnn-cu12; else echo ">>> CPU" && uv pip install --system -r pyproject.toml --all-extras; fi

RUN rm -rf /root/.cache/uv /tmp/*

# Chép mã nguồn local vào image (xem .dockerignore để biết những gì bị loại trừ)
COPY . .
