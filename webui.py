...
CHANNEL_SETTINGS = {
    ...
    "ChatGPT Dịch thuật": {
        ...
        "fields": [
            {"key": "chatgpt_api", "label": "API URL", "type": "text", "default": "", "placeholder": "Để trống dùng API chính thức"},
    ...
    "Local LLM": {
        "category": "Kênh dịch phụ đề",
        "fields": [
            {"key": "localllm_api", "label": "API URL", "type": "text", "default": "http://127.0.0.1:11434/v1", "placeholder": "VD: http://127.0.0.1:11434/v1"},
            {"key": "localllm_key", "label": "SK Key", "type": "text", "default": "no-key", "placeholder": "Thường để no-key"},
    ...
    "DeepL Dịch thuật": {
        ...
        "fields": [
            ...
            {"key": "deepl_api", "label": "API URL (bên thứ ba)", "type": "text", "default": "", "placeholder": "Để trống dùng API chính thức"},
    ...
    "ByteDance Volcano (VolcEngine)": {
        ...
        "fields": [
            ...
            {"key": "zijiehuoshan_model", "label": "Điểm接入推理", "type": "text", "default": "", "placeholder": "Nhập tên điểm接入"},
        ],
    },
    "MiniMax Dịch thuật": {
        "category": "Kênh dịch phụ đề",
    ...
    "OpenAI ASR": {
        ...
        "fields": [
            {"key": "openairecognapi_url", "label": "API URL", "type": "text", "default": "", "placeholder": "Để trống dùng API chính thức"},
    ...
    "OpenAI TTS": {
        ...
        "fields": [
            {"key": "openaitts_api", "label": "API URL", "type": "text", "default": "", "placeholder": "Để trống dùng API chính thức"},
    ...
    "Azure TTS": {
        ...
        "fields": [
            ...
            {"key": "azure_speech_region", "label": "Region / URL", "type": "text", "default": "eastasia", "placeholder": "VD: eastasia hoặc URL đầy đủ"},
        ],
    },
    "ElevenLabs TTS": {
        "category": "Kênh lồng tiếng",
    ...
    "CosyVoice TTS": {
        ...
        "fields": [
            ...
            {"key": "cosyvoice_instruct_text", "label": "Prompt hướng dẫn", "type": "text", "default": ""},
        ],
    },
    "OmniVoice TTS": {
        "category": "Kênh lồng tiếng",
...
# ---------------------------------------------------------------------------
# Trình chỉnh sửa kiểu ASS (Gradio thuần)
# ---------------------------------------------------------------------------
def build_ass_editor():
    ...
    with gr.Accordion("🎨 Chỉnh sửa kiểu phụ đề cứng", open=False):
        gr.Markdown("Sau khi sửa nhấn **Lưu kiểu**, kiểu sẽ áp dụng cho tất cả tác vụ nhúng phụ đề cứng.")
        with gr.Tabs():
            with gr.Tab("Phụ đề chính"):
                with gr.Row():
                    ass_fontname = gr.Textbox(label="Tên font", value=style.get('Fontname', 'Arial'))
            ...
            with gr.Tab("Phụ đề dưới (song ngữ)"):
                with gr.Row():
                    ass_bottom_fontname = gr.Textbox(label="Tên font", value=style.get('Bottom_Fontname', 'Arial'))
            ...
            with gr.Tab("Kiểu toàn cục"):
                with gr.Row():
                    ass_border_style = gr.Dropdown(label="Kiểu viền", choices=["Viền", "Nền đục"], value="Viền" if style.get('BorderStyle', 1) == 1 else "Nền đục")
                ...
                with gr.Row():
                    ass_scale_x = gr.Slider(label="Co giãn ngang %", minimum=1, maximum=1000, value=style.get('ScaleX', 100), step=1)
                    ass_scale_y = gr.Slider(label="Co giãn dọc %", minimum=1, maximum=1000, value=style.get('ScaleY', 100), step=1)
                    ass_spacing = gr.Slider(label="Khoảng cách chữ", minimum=-100, maximum=100, value=style.get('Spacing', 0), step=1)
                ...
                ass_alignment = gr.Dropdown(label="Vị trí căn chỉnh", choices=["Dưới-trái", "Dưới-giữa", "Dưới-phải", "Giữa-trái", "Giữa", "Giữa-phải", "Trên-trái", "Trên-giữa", "Trên-phải"],
                    value={1: "Dưới-trái", 2: "Dưới-giữa", 3: "Dưới-phải", 4: "Giữa-trái", 5: "Giữa", 6: "Giữa-phải", 7: "Trên-trái", 8: "Trên-giữa", 9: "Trên-phải"}.get(style.get('Alignment', 2), "Dưới-giữa"))
        with gr.Row():
        ...
        def save_ass_style(fontname, fontsize, primary_color, outline_color, back_color, bold, italic, underline, strikeout,
            ...
            am = {"Dưới-trái": 1, "Dưới-giữa": 2, "Dưới-phải": 3, "Giữa-trái": 4, "Giữa": 5, "Giữa-phải": 6, "Trên-trái": 7, "Trên-giữa": 8, "Trên-phải": 9}
            _save_ass_style({
                ...
                'BorderStyle': 1 if border_style == "Viền" else 3, 'Outline': float(outline), 'Shadow': float(shadow),
        ...
        def reset_ass_style():
            ...
            return (s['Fontname'], s['Fontsize'], _parse_ass_color(s['PrimaryColour']), _parse_ass_color(s['OutlineColour']),
                    ...
                    "Viền" if s['BorderStyle'] == 1 else "Nền đục",
                    s['Outline'], s['Shadow'], s['ScaleX'], s['ScaleY'], s['Spacing'], s['Angle'],
                    s['MarginL'], s['MarginR'], s['MarginV'],
                    {1: "Dưới-trái", 2: "Dưới-giữa", 3: "Dưới-phải", 4: "Giữa-trái", 5: "Giữa", 6: "Giữa-phải", 7: "Trên-trái", 8: "Trên-giữa", 9: "Trên-phải"}.get(s['Alignment'], "Dưới-giữa"),
...
def build_channel_settings():
    """Xây dựng tất cả bảng cài đặt kênh"""
    import gradio as gr
    ...
    with gr.Tabs():
            ...
            with gr.Tab(cat_name):
                for ch_name, ch_cfg in channels:
                    with gr.Accordion(ch_name, open=False):
                        ...
                        # Dùng closure để capture giá trị hiện tại
        ...
        with gr.Tab("Cài đặt âm thanh tham chiếu"):
            gr.Markdown("### Cài đặt âm thanh tham chiếu cho clone giọng nói")
            gr.Markdown(
                "Cấu hình âm thanh tham chiếu cho clone giọng nói. Mỗi dòng một mục, định dạng: `tên_file.wav#nội dung thoại trong file`\n"
                f"- File âm thanh cần đặt trong thư mục `{ROOT_DIR}/f5-tts/`\n"
                "- Định dạng file phải là wav\n"
                "- Mỗi dòng dùng `#` để phân cách tên file và nội dung tương ứng"
            )

            ref_audio_text = gr.Textbox(
                label="Danh sách âm thanh tham chiếu",
                value=str(_safe_get("f5tts_role", "")),
                placeholder="myaudio1.wav#Bạn nói tứ đại giai không, sao lại nhắm chặt hai mắt\nmyaudio2.wav#Hello, this is a test audio",
                lines=8,
                interactive=True,
            )
            ...
            def save_ref_audio(text):
                ...
                for i, line in enumerate(lines):
                    ...
                    parts = line.split("#")
                    if len(parts) != 2:
                        errors.append(f"Dòng {i+1} sai định dạng, cần dùng # để phân cách tên file và nội dung")
                        continue

                    filename = parts[0].strip()
                    f5tts_dir = Path(ROOT_DIR) / "f5-tts"

                    # Kiểm tra file tồn tại (có hoặc không có đuôi .wav)
                    if not (f5tts_dir / filename).exists() and not (f5tts_dir / f"{filename}.wav").exists():
                        errors.append(f"Dòng {i+1}: File `{filename}` không tồn tại trong thư mục f5-tts/")
                        continue
...
# Các key prompt Whisper và nhãn
_prompt_keys_list = [
...
# Registry widget toàn cục
_all_widgets = {}


def _w(key, label, tip="", area=False):
    """Tạo một mục cài đặt: tiêu đề ở trên, component ở dưới"""
    import gradio as gr
...
def _save_section(section_key, keys):
    """Tạo nút lưu và hiển thị trạng thái cho một phần cụ thể"""
    import gradio as gr
    with gr.Row():
...
# ---------------------------------------------------------------------------
# Bảng tùy chọn nâng cao (bố cục lưới gọn)
# ---------------------------------------------------------------------------
ADVANCED_SECTION_TITLES = {
    "common": "Cài đặt chung", "video": "Điều khiển đầu ra video", "whisper": "Tham số nhận dạng giọng nói",
    "trans": "Tinh chỉnh dịch phụ đề", "dubbing": "Tinh chỉnh lồng tiếng",
    "justify": "Căn chỉnh phụ đề-âm thanh-hình ảnh", "prompt_init": "Prompt mô hình Whisper",
}
...
def build_advanced_settings():
    ...
    with gr.Accordion("📋 Cài đặt chung", open=True):
        with gr.Row():
            _w("lang", "Ngôn ngữ giao diện", "Cần khởi động lại")
            _w("countdown_sec", "Đếm ngược tạm dừng mỗi video", "Đặt 0 để bỏ qua cửa sổ chỉnh sửa")
            _w("retry_nums", "Số lần thử lại khi thất bại", "")
        with gr.Row():
            _w("llm_chunk_size", "Số dòng phụ đề mỗi lần LLM xử lý", "Mặc định 20")
            _w("llm_ai_type", "Kênh AI cho LLM ngắt câu", "chatgpt/deepseek")
            _w("batch_nums", "Số lượng mỗi đợt", "0=không giới hạn")
        with gr.Row():
            _w("dont_notify", "Tắt thông báo desktop", "")
            _w("show_more_settings", "Hiển thị tất cả tham số trên màn hình chính?", "")
            _w("homedir", "Thư mục đầu ra tính năng độc lập", "")
        with gr.Row():
            _w("process_max", "Số tác vụ CPU [cần restart]", "Không vượt quá số lõi CPU")
            _w("process_max_gpu", "Số tác vụ GPU [cần restart]", "Chỉ >1 nếu multi-GPU hoặc VRAM>24GB")
            _w("multi_gpus", "Chế độ multi-GPU [cần restart]", "")
    ...
    with gr.Accordion("📋 Điều khiển đầu ra video", open=False):
        with gr.Row():
            _w("crf", "Chất lượng video (0=lossless, 51=kém)", "")
            _w("preset", "Tỷ lệ nén", "ultrafast→veryslow")
            _w("video_codec", "Mã hóa 264/265", "")
        with gr.Row():
            _w("out_video_ext", "Định dạng đầu ra", "mp4/mkv")
            _w("fps_mode", "Chế độ FPS", "vfr/cfr")
            _w("force_lib", "Ép dùng soft encoding?", "")
        with gr.Row():
    ...
    with gr.Accordion("📋 Tham số nhận dạng giọng nói", open=False):
        with gr.Row():
            ...
            _w("no_speech_threshold", "Ngưỡng không phải giọng nói", "")
        with gr.Row():
            _w("max_speech_duration_s", "Giọng nói dài nhất (giây)", "")
            _w("min_speech_duration_ms", "Giọng nói ngắn nhất (ms)", "")
            _w("min_silence_duration_ms", "Im lặng tối thiểu để cắt (ms)", "")
        with gr.Row():
            _w("max_speech_duration_s2", "Nhận dạng lần 2 - dài nhất (giây)", "")
            _w("min_speech_duration_ms2", "Nhận dạng lần 2 - ngắn nhất (ms)", "")
            _w("merge_short_sub", "Gộp phụ đề quá ngắn", "")
        with gr.Row():
            _w("whisper_prepare", "Whisper pre-split?", "Chọn khi dùng clone giọng nói")
            _w("speaker_type", "Mô hình tách người nói", "built/pyannote")
        ...
        with gr.Row():
            _w("condition_on_previous_text", "Nhận biết ngữ cảnh", "")
            _w("repetition_penalty", "Phạt lặp", "")
            _w("compression_ratio_threshold", "Tỷ lệ nén văn bản", "")
        with gr.Row():
            _w("temperature", "Nhiệt độ sampling", "")
            _w("hotwords", "Từ nóng", "Phân cách bằng dấu phẩy")
            _w("gemini_recogn_chunk", "Số lượng chunk Gemini", "")
        with gr.Row():
            _w("zh_hant_s", "Chuyển phồn thể→giản thể", "")
    ...
    # ---- Tinh chỉnh dịch phụ đề ----
    with gr.Accordion("📋 Tinh chỉnh dịch phụ đề", open=False):
        with gr.Row():
            _w("trans_thread", "Số dòng mỗi lần dịch truyền thống", "")
            _w("aitrans_thread", "Số dòng mỗi lần dịch AI", "")
            _w("aitrans_temperature", "Nhiệt độ AI", "Mặc định 1.0")
        with gr.Row():
            _w("translation_wait", "Tạm dừng sau dịch (giây)", "")
            _w("aisendsrt", "Gửi toàn bộ phụ đề", "")
            _w("aitrans_context", "Dịch tất cả dòng một lần", "Cần mô hình có ngữ cảnh siêu dài")
        _save_section("trans", ["trans_thread", "aitrans_thread", "aitrans_temperature",
                                 "translation_wait", "aisendsrt", "aitrans_context"])

    # ---- Tinh chỉnh lồng tiếng ----
    with gr.Accordion("📋 Tinh chỉnh lồng tiếng", open=False):
        with gr.Row():
        ...
        with gr.Row():
            _w("save_segment_audio", "Giữ lại file lồng tiếng từng dòng", "")
            _w("normal_text", "Chuẩn hóa văn bản", "")
            _w("chattts_voice", "Giá trị âm sắc ChatTTS", "")
        with gr.Row():
        ...
        with gr.Row():
            _w("uvr_models", "Mô hình tách nhạc nền", "")
    ...
    # ---- Căn chỉnh phụ đề-âm thanh-hình ảnh ----
    with gr.Accordion("📋 Căn chỉnh phụ đề-âm thanh-hình ảnh", open=False):
        with gr.Row():
            _w("max_audio_speed_rate", "Tốc độ tăng âm thanh tối đa", "Mặc định 100")
            _w("max_video_pts_rate", "Tốc độ chậm video tối đa", "Mặc định 10, ≤10")
        with gr.Row():
            _w("cjk_len", "Số ký tự/dòng cho phụ đề CJK", "")
            _w("other_len", "Số ký tự/dòng cho phụ đề ngôn ngữ khác", "")
        _save_section("justify", ["max_audio_speed_rate", "max_video_pts_rate", "cjk_len", "other_len"])

    # ---- Prompt mô hình Whisper ----
    with gr.Accordion("📋 Prompt mô hình Whisper", open=False):
...
>  [Báo lỗi](https://bbs.pyvideotrans.com)
----
        """)
            ...
            with gr.Tab("🎬 Dịch video", id="translate"):
                ...
                with gr.Row():
                    with gr.Column(scale=3):
                        input_file = gr.Video(label="Chọn file video", interactive=True)
                        ...
                        translate_choice = gr.Dropdown(choices=TRANSLATE_NAMES, value=TRANSLATE_NAMES[int(_user_params.get('translate_type', DEFAULT_TRANSLATE)) if str(_user_params.get('translate_type', '')).isdigit() else DEFAULT_TRANSLATE], label="Kênh dịch thuật", interactive=True)
                        source_lang = gr.Dropdown(choices=LANG_DISPLAY_NAMES, value=_user_params.get('source_language', DEFAULT_SOURCE_LANG), label="Ngôn ngữ nguồn", interactive=True)
                        target_lang = gr.Dropdown(choices=['-']+LANG_DISPLAY_NAMES, value=_user_params.get('target_language', DEFAULT_TARGET_LANG), label="Ngôn ngữ đích", interactive=True)
                        ...
                        tts_choice = gr.Dropdown(choices=TTS_NAMES, value=TTS_NAMES[int(_user_params.get('tts_type', DEFAULT_TTS)) if str(_user_params.get('tts_type', '')).isdigit() else DEFAULT_TTS], label="Kênh lồng tiếng", interactive=True)
                                                # Pre-fill role list based on loaded TTS channel and target language
                        ...
                        voice_role = gr.Dropdown(choices=_init_roles, value=_init_role_val, label="Giọng đọc", interactive=True)

                        with gr.Row():
                            voice_autorate = gr.Checkbox(label="Tăng tốc giọng đọc", value=True)
                            video_autorate = gr.Checkbox(label="Giảm tốc video", value=False)
                        with gr.Row():
                            voice_rate = gr.Slider(minimum=-50, maximum=50, value=int(str(_user_params.get("voice_rate", "0")).replace("%","")), step=1, label="Tốc độ giọng đọc (%)")
                        ...
                        with gr.Accordion("📋 Cài đặt thêm", open=False):
                            ...
                            with gr.Row():
                                is_separate = gr.Checkbox(label="Tách giọng nói & nhạc nền", value=False)
                                embed_bgm = gr.Checkbox(label="Nhúng lại nhạc nền", value=True)
                            with gr.Row():
                                loop_bgm = gr.Dropdown(choices=list(LOOP_BGM_OPTIONS.keys()), value="Cắt nhạc nền", label="Xử lý nhạc nền", interactive=True)
                                backaudio_volume = gr.Slider(minimum=0.0, maximum=2.0, value=float(_user_params.get("backaudio_volume", settings.get("backaudio_volume", 0.8))), step=0.1, label="Âm lượng nhạc nền")

                        cuda_accel = gr.Checkbox(label="Bật CUDA tăng tốc", value=False)
                        channel_warning = gr.Markdown("", visible=False)
                        
                        start_btn = gr.Button("🚀 Bắt đầu xử lý", variant="primary", size="lg")

                    with gr.Column(scale=2):
                        log_output = gr.Textbox(label="Nhật ký", lines=20, interactive=False)
                        video_preview = gr.Video(label="Xem trước video", interactive=False)
                        result_files = gr.File(label="File đầu ra (click để tải)", interactive=False)
                ...
                def validate_recogn(choice, prev):
                    ...
                    if _rs is not True:
                        msg = "Kênh {} không khả dụng, đã tự động quay lại".format(choice)
                        gr.Warning(msg)
                        return prev, f"⚠️ {msg}"
                    return choice, ""
                ...
                # Thực thi dịch
                _BTN_RUNNING = gr.update(value="⏳ Đang xử lý...", interactive=False)
                _BTN_IDLE = gr.update(value="🚀 Bắt đầu xử lý", interactive=True)
                ...
                def run_translation(file_path, recogn_display, model_name, translate_display,
                    ...
                    print(f'{file_path=}')
                    if not file_path:
                        yield "❌ Vui lòng chọn một file video hoặc âm thanh", None, [], _BTN_IDLE
                        return
                    app_cfg.current_status = 'ing'
                    # Xóa log, preview và output cũ, hiển thị trạng thái đang xử lý
                    ...
                    try:
                        ...
                        yield log(f"File nguồn: {Path(file_path).name}"), None, [], _BTN_RUNNING
                        ...
                        yield log(f"Ngôn ngữ: {source_code} → {target_code}   Giọng: {voice_role_name}"), None, [], _BTN_RUNNING
                        ...
                        stages = [
                            ("GĐ 1/8: Tiền xử lý...", "prepare", "Tiền xử lý hoàn tất"),
                            ("GĐ 2/8: Nhận dạng giọng nói...", "recogn", "Nhận dạng giọng nói hoàn tất"),
                            ("GĐ 3/8: Tách người nói...", "diariz", "Tách người nói hoàn tất"),
                            ("GĐ 4/8: Dịch phụ đề...", "trans", "Dịch phụ đề hoàn tất"),
                            ("GĐ 5/8: Tạo giọng đọc...", "dubbing", "Tạo giọng đọc hoàn tất"),
                            ("GĐ 6/8: Căn chỉnh âm-hình...", "align", "Căn chỉnh âm-hình hoàn tất"),
                            ("GĐ 7/8: Nhận dạng lần 2...", "recogn2pass", "Nhận dạng lần 2 hoàn tất"),
                            ("GĐ 8/8: Tổng hợp cuối...", "assembling", "Tổng hợp cuối hoàn tất"),
                        ]
                        ...
                        trk.task_done()
                        yield log("✓ Tổng hợp video hoàn tất"), None, [], _BTN_RUNNING
                        yield log("✅ Tất cả tác vụ đã hoàn tất!"), None, [], _BTN_RUNNING
                            ...
                            for f in sorted(Path(_cache_folder).rglob("*")):
                                if f.is_file():
                                    if f.suffix.lower() == '.mp4' and video_preview_path is None:
                                        ...
                                        output_files.append(str(f))
                        # Thêm file log hôm nay vào danh sách đầu ra
                        import datetime
                        ...
                        yield log(f"❌ Lỗi thực thi: {str(e)}\n\n{tb}"), None, [], _BTN_IDLE
...
if __name__ == "__main__":
    try:
        ...
        /* Mặc định: Microsoft YaHei > PingFang SC > sans-serif */
        *, *::before, *::after {
        ...
        /* Đồng bộ font cho input, textarea, select, button, label */
