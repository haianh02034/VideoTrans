...
from phiendichvideo.configure.config import ROOT_DIR, tr, settings, logger, HOME_DIR
from phiendichvideo.configure import config
from phiendichvideo.recognition import run
...
class SpeechToText(BaseTask):
    ...
    def recogn(self):
        ...
        from phiendichvideo.util.help_down import down_file_from_ms
        from phiendichvideo.configure.excepts import SpeechToTextError

        # 需要降噪
        if self.cfg.remove_noise:
            logger.debug('开始降噪')
            from phiendichvideo.process.prepare_audio import remove_noise
        ...
        if self.cfg.detect_language != 'auto' and self.cfg.fix_punc==1 and self.cfg.detect_language[:2] in ['zh', 'en']:
            from phiendichvideo.process.prepare_audio import fix_punc
        ...
        if not self.cfg.enable_diariz and self.cfg.rephrase == 1:
            # LLM重新断句
            try:
                from phiendichvideo.translator._openaicompat import OpenAICampat
                ob = OpenAICampat(
    ...
    def diariz(self):
        if self._exit() or not self.cfg.enable_diariz or Path(self.cfg.cache_folder + "/speaker.json").exists():
            return
        from phiendichvideo.util.help_down import down_file_from_ms, check_and_down_ms
        ...
        if speaker_type == 'built':
            ...
            from phiendichvideo.process.prepare_audio import built_speakers as _run_speakers
            del kw['is_cuda']
            ...
            check_and_down_ms(model_id='iic/speech_campplus_speaker-diarization_common',
                                    callback=self._process_callback)
            from phiendichvideo.process.prepare_audio import cam_speakers as _run_speakers
        elif speaker_type == 'pyannote':
            from phiendichvideo.process.prepare_audio import pyannote_speakers as _run_speakers
        elif speaker_type == 'reverb':
            from phiendichvideo.process.prepare_audio import reverb_speakers as _run_speakers
        else:
    ...
    def task_done(self):
        if self._exit(): return
        from phiendichvideo.util.help_srt import simple_wrap
        ...
        if self.cfg.fix_punc==2:
            from phiendichvideo.util.help_srt import delete_punc
            for i, it in enumerate(self.source_srt_list):
