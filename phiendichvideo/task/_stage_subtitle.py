...
from phiendichvideo.util.help_srt import get_subtitle_from_srt, simple_wrap, set_ass_font
...
class SubtitleMixin:
    ...
    def _get_join_flag(self):
        _join_flag = ""
        if self.cfg.subtitle_type != 3 or not Path(f'{ROOT_DIR}/phiendichvideo/ass.json').exists():
            return _join_flag
        try:
            assjson = json.loads(Path(f'{ROOT_DIR}/phiendichvideo/ass.json').read_text(encoding='utf-8'))
        except Exception:
