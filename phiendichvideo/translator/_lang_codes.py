...
from phiendichvideo.configure.config import tr, ROOT_DIR, logger
...
# 如果存在新增
try:
    if Path(ROOT_DIR + f'/phiendichvideo/newlang.txt').exists():
        _new_lang = Path(ROOT_DIR + f'/phiendichvideo/newlang.txt').read_text().strip().split("\n")
        for nl in _new_lang:
            LANGNAME_DICT[nl] = nl
