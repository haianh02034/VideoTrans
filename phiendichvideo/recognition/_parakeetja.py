...
import time
from phiendichvideo.configure.config import params, logger, settings,ROOT_DIR
from phiendichvideo.configure.excepts import SpeechToTextError, StopTask
...
class ParakeetJARecogn(BaseRecogn):
    ...
    def _download(self):
        if not Path(f'{ROOT_DIR}/models/parakeet/model.int8.onnx').exists():
            from phiendichvideo.util import help_down
