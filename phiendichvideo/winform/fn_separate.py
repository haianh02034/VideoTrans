...
def openwin():
    ...
    from phiendichvideo.configure.config import tr,app_cfg, params, HOME_DIR
    # 分离背景音
    ...
    def start():
        ...
        # 开始分离
        from phiendichvideo.task.separate_worker import SeparateWorker
        winobj.task = SeparateWorker(parent=winobj, file=file, out=outdir, uuid=uuid)
        winobj.task.finish_event.connect(update)
        winobj.task.start()
    
    from phiendichvideo.component.set_form import SeparateForm
