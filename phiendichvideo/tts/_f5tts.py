...
from phiendichvideo.configure.config import ROOT_DIR, app_cfg, logger,tr
from phiendichvideo.configure.excepts import DubbingSrtError
from phiendichvideo.util import tools,gpus
...
class F5TTSBuilt(BaseTTS):
    ...
    def __post_init__(self):
        ...
        cfg=json.loads(Path(f'{ROOT_DIR}/phiendichvideo/voicejson/f5ttscfg.json').read_text(encoding='utf-8'))
        if language not in cfg or not Path(f'{ROOT_DIR}/phiendichvideo/voicejson/f5ttscfg/{language}.yaml').is_file():
    ...
    def _exec(self):
        ...
        f5tts = _F5TTS(
            yaml_path=f"{ROOT_DIR}/phiendichvideo/voicejson/{self.cfg['config']}",
