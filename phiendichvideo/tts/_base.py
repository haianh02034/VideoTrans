...
from phiendichvideo.configure.config import tr, settings, logger, ROOT_DIR
from phiendichvideo.configure import config
from phiendichvideo.util.help_misc import vail_file,pygameaudio,get_tts_type
...
class BaseTTS(BaseCon):
    ...
    def _local_mul_thread(self) -> None:
        if self._exit(): return
        from phiendichvideo.configure.excepts import StopTask
        # 单个字幕行，无需多线程
    ...
    def _cleantts(self) -> None:
        ...
        if settings.get('normal_text'):
            if self.language[:2] == 'zh':
                from phiendichvideo.util.cn_tn import TextNorm
                normalizer = TextNorm(to_banjiao=True)
            elif self.language[:2] == 'en':
                from phiendichvideo.util.en_tn import EnglishNormalizer
