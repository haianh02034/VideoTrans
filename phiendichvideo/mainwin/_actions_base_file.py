...
from phiendichvideo.configure.config import tr, settings, params, app_cfg, ROOT_DIR, TEMP_ROOT
from phiendichvideo.configure import contants
from phiendichvideo.util.help_misc import show_popup, set_proxy, set_process
...
class WinActionBaseFileMixin:
    ...
    def proxy_alert(self):
        from phiendichvideo.component.set_proxy import SetThreadProxy
    ...
    def clearcache(self):
        question = show_popup(tr('Confirm cleanup?'),
                                    tr('After cleaning, you need to restart the software. Only cache and temporary files are cleaned. For configuration information, please directly delete the .json in the phiendichvideo folder.'))
    ...
    def _clean_dir(self):
        ...
        Path(ROOT_DIR + "/phiendichvideo/codec.json").unlink(missing_ok=True)
        Path(ROOT_DIR + "/phiendichvideo/ass.json").unlink(missing_ok=True)
