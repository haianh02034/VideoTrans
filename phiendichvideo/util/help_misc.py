...
from phiendichvideo.configure.config import tr, app_cfg, logger, ROOT_DIR, defaulelang, push_queue
from phiendichvideo.task.taskcfg import SignMsg
...
def show_popup(title, text):
    ...
    msg.setWindowIcon(QIcon(f"{ROOT_DIR}/phiendichvideo/styles/icon.ico"))
    msg.setText(text)
...
def show_error(tb_str):
    ...
    icon_path = f"{ROOT_DIR}/phiendichvideo/styles/icon.ico"
    try:
...
def get_prompt(ainame, aisendsrt=True):
    ...
    glossary = ''
    if Path(ROOT_DIR + '/phiendichvideo/glossary.txt').exists():
        glossary = Path(ROOT_DIR + '/phiendichvideo/glossary.txt').read_text(encoding='utf-8-sig', errors="ignore").strip()
        if glossary:
...
def get_prompt_file(ainame, aisendsrt=True):
    prompt_path = f'{ROOT_DIR}/phiendichvideo/'
...
def show_glossary_editor(parent):
    ...
    # 读取文件内容，并设置为文本框默认值
    file_path = ROOT_DIR + "/phiendichvideo/glossary.txt"
    try:
...
def get_recogn_type(type_index=None):
    from phiendichvideo.recognition import RECOGN_NAME_LIST
...
def get_tts_type(type_index=None):
    from phiendichvideo.tts import TTS_NAME_LIST
...
def show_refaudio_win():
    from phiendichvideo.component.set_form import RefaudioForm
    dialog = RefaudioForm()
    dialog.exec()
    return
