...
from phiendichvideo.configure.config import ROOT_DIR, tr, settings, params, logger
...
def get_camb_role(force=False, raise_exception=False):
    ...
    jsonfile = f'{ROOT_DIR}/phiendichvideo/voicejson/camb.json'
    namelist = ["No", "clone"]
...
def get_elevenlabs_role(force=False, raise_exception=False):
    from . import help_misc
    jsonfile = f'{ROOT_DIR}/phiendichvideo/voicejson/elevenlabs.json'
    namelist = ["No"]
    if help_misc.vail_file(jsonfile):
        with open(jsonfile, 'r', encoding='utf-8-sig') as f:
            cache = json.loads(f.read())
...
@lru_cache
def get_piper_role():
    file_path = f"{ROOT_DIR}/phiendichvideo/voicejson/piper.json"
...
def get_302ai():
    ...
    with open(ROOT_DIR + "/phiendichvideo/voicejson/302.json", 'r', encoding='utf-8-sig') as f:
...
def get_doubao2_rolelist(role_name=None, langcode="zh"):
    roledata = json.loads(Path(f'{ROOT_DIR}/phiendichvideo/voicejson/doubao2.json').read_text(encoding='utf-8-sig'))
...
def get_edge_rolelist(role_name=None, locale=None):
    from . import help_misc
    voice_list = {}
    voice_file = ROOT_DIR + "/phiendichvideo/voicejson/edge_tts.json"
...
def get_azure_rolelist(language=None, role_name=None):
    voice_file = ROOT_DIR + "/phiendichvideo/voicejson/azure_voice_list.json"
...
def get_minimaxi_rolelist():
    from . import help_misc
    voice_list = {}
    voice_file = ROOT_DIR + "/phiendichvideo/voicejson/minimaxi.json"

    if params.get("minimaxi_apiurl", '') == 'api.minimax.io':
        voice_file = ROOT_DIR + "/phiendichvideo/voicejson/minimaxiio.json"
    if help_misc.vail_file(voice_file):
        try:
            with open(voice_file, 'r', encoding='utf-8-sig') as f:
                voice_list = json.loads(f.read())
...
def get_qwen3tts_rolelist():
    voices = json.loads(Path(ROOT_DIR + "/phiendichvideo/voicejson/qwen3tts.json").read_text(encoding='utf-8-sig'))
    voices = {"No": "No"} | voices
    return voices
...
def get_supertonic_rolelist():
    voices = json.loads(Path(ROOT_DIR + "/phiendichvideo/voicejson/supertonic.json").read_text(encoding='utf-8-sig'))
    voices = {"No": "No"} | voices
    return voices

@lru_cache
def get_glmtts_rolelist():
    voices = json.loads(Path(ROOT_DIR + "/phiendichvideo/voicejson/glmtts.json").read_text(encoding='utf-8-sig'))
    voices = {"No": "No"} | voices
    return voices
