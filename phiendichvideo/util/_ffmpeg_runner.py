...
from phiendichvideo.configure.config import ROOT_DIR, tr, app_cfg, settings, logger
...
def runffmpeg(arg, *, noextname=None, force_cpu=True, cmd_dir=None):
    ...
    try:
        ...
        if sys.platform == 'win32' and 'No such file or directory' in str(e):
            ...
            err = _err or err
        from phiendichvideo.configure.excepts import FFmpegError
...
def get_filepath_from_cmd(cmd: list):
    ...
    for file in file_list:
        if len(file) >= 255:
            return tr(
                'The file path and file name may be too long. Please move the file to a flat and short directory and rename the file to a shorter name, ensuring that the length from the drive letter to the end of the file name does not exceed 255 characters: {},For example D:/phiendichvideo/1.mp4 D:/phiendichvideo/2.wav',
                file)
        for flag in special:
            if flag in file:
                return tr(
                    'There may be special characters in the file name or path. Please move the file to a simple directory consisting of English and numerical characters, rename the file to a simple name, and try again to avoid errors: {},For example D:/phiendichvideo/1.mp4 D:/phiendichvideo/2.wav',
                    file)
    return None
