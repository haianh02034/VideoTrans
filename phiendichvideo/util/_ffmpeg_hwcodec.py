...
from phiendichvideo.configure.config import ROOT_DIR, app_cfg, settings, logger
...
def get_video_codec(compat=None) -> str:
    ...
    try:
        if not _codec_cache and Path(f'{ROOT_DIR}/phiendichvideo/codec.json').exists():
            _codec_cache = json.loads(Path(f'{ROOT_DIR}/phiendichvideo/codec.json').read_text(encoding='utf-8-sig'))
    ...
    try:
        test_input_file = Path(ROOT_DIR) / "phiendichvideo/styles/no-remove.mp4"
    ...
    if not encoders_to_test:
        ...
        try:
            ...
            Path(f"{ROOT_DIR}/phiendichvideo/codec.json").write_text(json.dumps(_codec_cache))
        except Exception as e:
